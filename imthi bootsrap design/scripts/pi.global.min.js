...
function getElementHeight($el) {
    ...
    return "none" == display || "none !important" == display ? ($el.css({
        visibility: "hidden",
        display: "block"
    }), h = $el.outerHeight(), $el.css({
        visibility: vis,
        display: display
    })) : h = $el.outerHeight(), h
}
...
function PiImagesLoader($el, callback, delay) {
    var o = {},
        $ = jQuery;
    o.$el = $el ? "string" == typeof $el ? jQuery($el) : $el : 0, o.$imgs = o.$el.find("img"), o.imgsQ = o.$imgs.length, o.imgsQ_loaded = 0, o.delay = delay ? delay : 0, o.started = 0, o.loaderShown = 0, o.$loadingAnimation = 0, o.$dots = 0, o.s = {
        c: {}
    }, o.s.dotDistance = 14, o.s.dotQuantity = 3, o.s.dotAnimationSpeed = 100, o.s.c.loading = "pi-loader", o.s.c.dot = "pi-loader-dot", o.s.c.dotActive = "pi-loader-dot-active", o.callback = callback && "function" == typeof callback ? callback : function() {}, o.addLoadingAnimation = function() {
        o.loaderShown = !0;
...
jQuery(function($) {
    ...
    function checkHeaderBlocks() {
        for (var i = 0; i < hrObjects.length; i++) {
            ...
            $firstVisibleBlock.length < 1 || $curHr.currentBlock && $curHr.currentBlock.get(0) == $firstVisibleBlock.get(0) || ($curHr.currentBlock && $curHr.currentBlock.css({
                marginRight: $curHr.currentBlock.get(0).initialMarginRight + "px"
            }), $curHr.currentBlock = $firstVisibleBlock, $curHr.currentBlock.get(0).initialMarginRight || ($curHr.currentBlock.get(0).initialMarginRight = parseInt($firstVisibleBlock.css("margin-right"), 10)), $firstVisibleBlock.css({
                marginRight: 0
            }))
        }
    }
    ...
    function MobileMenus() {
        ...
        function toggleSubmenu(mobileMenu, $li) {
            $li.get(0).$list.hasClass("pi-active") ? (mobileMenu.height = mobileMenu.height - $li.get(0).listHeight, mobileMenu.$wrapper.height(mobileMenu.height), $li.get(0).$list.removeClass("pi-active"), $li.get(0).$list.animate({
                height: 0
            }, 500, function() {
                changeParentsListHeight($li, $li.get(0).listHeight, "take")
            })) : (mobileMenu.height = mobileMenu.height + $li.get(0).listHeight, mobileMenu.$wrapper.height(mobileMenu.height), $li.get(0).$list.addClass("pi-active"), $li.get(0).$list.animate({
        ...
        function toggleMenu(m) {
            m.$wrapper.toggleClass(settings.classOpen), m.$wrapper.hasClass(settings.classOpen) ? (m.$wrapper.height(m.height), m.$wrapper.css({
                overflow: "auto"
            }), m.state = 1) : (m.$wrapper.height(0), m.$wrapper.css({
                overflow: "none"
            }), m.state = 0)
        }
        var mobileMenus = [],
    ...
    if ($.fn.scrollTo) {
        ...
        $("[data-scroll-to]").each(function() {
            ...
            target = $(target).length ? $(target) : target, $el.bind("click", function(e) {
                $.scrollTo.window().stop(!0), offset = "lg" != window.piCurrentBound ? defaultOffsetMin : initialOffset, $w.scrollTo(target, duration, {
                    offset: offset,
                    easing: easing
                }), e.preventDefault()
            })
    ...
    $(".pi-jump a, .pi-jump-bg a").each(function() {
        var $el = $(this);
        $el.find("i").length <= 1 && $el.append($el.find("i").clone())
    }), $(".pi-social-icons-big a i").wrap("<span></span>")
}), jQuery(function($) {
    $(".pi-overlay-slide:not(.pi-caption-opened)").each(function() {
        var $caption = $(this),
            $parent = $caption.parents(".pi-img-w"),
            height = $caption.outerHeight(!0);
        $caption.hasClass("pi-show-heading") && (height -= $caption.find("h2,h3,h4,h5,h6").eq(0).outerHeight(!0) + parseInt($caption.css("padding-top"), 10)), $caption.css({
            bottom: -height,
            visibility: "visible"
    ...
    function piColorboxInit() {
        ...
        colorboxNeeded && !colorboxState ? ($colorboxLinks.each(function() {
            ...
            videoSize[0] ? (videoSize[0] = parseInt(videoSize[0], 10), videoSize[1] = parseInt(videoSize[1], 10), "number" == typeof videoSize[0] && "number" == typeof videoSize[1] && $el.colorbox({
                rel: group,
                iframe: !0,
    ...
    if ($.fn.colorbox) {
        var $d = $(document),
            $colorboxLinks = ($(window), $(".pi-colorbox")),
